...
export type ShareActivity = { name: string; emoji: string };
 ...
 *  `activity` (name + emoji) comes from the active skin rather than being
 *  hardcoded, so a future re-skin to a different sport doesn't require
 *  touching this file — an activity with no obvious emoji just leaves it "". */
export function buildShareTemplates(
  scenario: AreaScenario, activity: ShareActivity,
  ...
  // Renders as " 🏈" mid-sentence, or nothing at all — never a dangling space.
  const tag = activity.emoji ? ` ${activity.emoji}` : "";
  ...
  switch (scenario.kind) {
    ...
    case "ambient-interest": {
      ...
      const captionLede = scenario.viewerIncluded
        ? `${scenario.totalCount} people (including me) want a ${activityName} game in ${where}${tag}`
        : `${scenario.totalCount} people want a ${activityName} game in ${where}${tag}`;
      return [
    ...
    case "open-proposal":
      return [
        ...
        { label: "caption", text: `There's a ${activityName} game on the table in ${where}${tag}\n\n${scenario.interestedCount} of the ${scenario.pMin} we need are already in — help us get there: ${url}` },
      ];
    case "games":
      return [
        { label: "short", text: `There's a standing ${activityName} game in ${where}${scenario.placeText ? ` at ${scenario.placeText}` : ""} — come play: ${url}` },
        // The emoji doubles as the mid-sentence beat here; fall back to a dash
        // so an emoji-less skin doesn't read "now come fill out".
        { label: "caption", text: `Weekly pickup ${activityName} is a real thing in ${where} now${tag || " -"} come fill out the roster: ${url}` },
      ];
  }
}
